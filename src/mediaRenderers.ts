    ...
    registerRenderer('slideshow', (props) => {
      ...
      useEffect(() => {
        ...
        const timer = setInterval(() => {
          setCurrentIndex((prev: number) => (prev + 1) % sources.length)
        }, interval)
      ...
      return h(
        'div',
        { style: { position: 'relative', width: '100%', height: props.height } },
        h('img', {
        ...
        props.showArrows && sources.length > 1
          ? h(
              ...
              h(
                'button',
                {
                  onClick: () => setCurrentIndex((prev: number) => (prev - 1 + sources.length) % sources.length),
                  style: { ...ARROW_BUTTON_STYLE, left: 10 },
                },
                '‹',
              ),
              h(
                'button',
                {
                  onClick: () => setCurrentIndex((prev: number) => (prev + 1) % sources.length),
                  style: { ...ARROW_BUTTON_STYLE, right: 10 },
                },
                '›',
              ),
            )
