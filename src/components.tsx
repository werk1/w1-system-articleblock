...
export function ArticleMenuComponent({
  ...
  const items = useMemo(
    () => buildInlineItems(block, preloadedArticles),
    [block, preloadedArticles],
  )
...
export function HeroRenderer({ hero, isMobile = false }: HeroRendererProps) {
  if (hero.mimeType?.startsWith('video/')) {
    return <video src={hero.url} controls style={{ width: '100%', height: 'auto', display: 'block' }} />
  }

  if (hero.mimeType?.startsWith('audio/')) {
    return (
      <audio src={hero.url} controls style={{ width: '100%', display: 'block' }}>
...
export function DefaultArticleBlock({ block, deviceInfo, isMobile = false }: DefaultArticleBlockProps) {
  ...
  const articleKeys = useMemo(
    () => new Set(block.articles.map((article) => article.article.slug)),
    [block.articles],
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
      ...
      {activeArticle.hero ? <HeroRenderer hero={activeArticle.hero} isMobile={isMobile} /> : null}

      <ArticleBody article={activeArticle as NormalizedArticleRenderData} deviceInfo={deviceInfo} isMobile={isMobile} />
    </div>
  )
}
...
export function ArchiveArticleBlock({ block, deviceInfo, isMobile = false }: VariantArticleBlockProps) {
  return (
    ...
    >
      <div style={{ width: '100%', padding: '0 1rem 1rem', color: '#3b3b39', fontSize: '0.8rem', fontWeight: 600 }}>
        Archive Block
      </div>
      <DefaultArticleBlock block={block} deviceInfo={deviceInfo} isMobile={isMobile} />
    </section>
  )
}
...
export function StoryArticleBlock({ block, deviceInfo, isMobile = false }: VariantArticleBlockProps) {
  return (
    ...
    >
      <div style={{ width: '100%', padding: '0 1rem 1rem', color: '#5f5646', letterSpacing: '0.08em', textTransform: 'uppercase', fontSize: '0.75rem' }}>
        Story Block
      </div>
      <DefaultArticleBlock block={block} deviceInfo={deviceInfo} isMobile={isMobile} />
    </section>
